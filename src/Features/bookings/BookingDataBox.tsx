...
function BookingDataBox({ booking }: { booking: BookingType }) {
  const {
    ...
    observations,
    isPaid,
    guests: { fullName: guestName, nationality: country, email, countryFlag, nationalID },
  ...
  return (
    <StyledBookingDataBox>
      ...
      <Section>
        <Guest>
          {countryFlag && <Flag alt={`Flag of ${country}`} src={countryFlag} />}
          <p>
        ...
        {observations && (
          <DataItem label="Observations" icon={<HiOutlineChatBubbleBottomCenterText />}>
            {observations}
          </DataItem>
        )}

        <DataItem label="Breakfast included?" icon={<HiOutlineCheckCircle />}>
          {hasBreakfast ? "Yes" : "No"}
        </DataItem>

        <Price $isPaid={isPaid}>
          <DataItem label={`Total price`} icon={<HiOutlineCurrencyDollar />}>
