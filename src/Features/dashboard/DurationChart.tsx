...
interface Stay extends BookingType {}
...
function prepareData(startData: DurationData[], stays: Stay[]): DurationData[] {
  ...
  const data = stays
    .reduce((arr: DurationData[], cur: Stay) => {
      ...
      <ResponsiveContainer height={240} width="100%">
        <PieChart>
          <Pie
            ...
            outerRadius={110}
          >
            {data.map((entry: DurationData) => (
              <Cell fill={entry.color} key={entry.duration} stroke={entry.color} />
            ))}
          </Pie>
          <Tooltip />
          <Legend
